...
#endif // ENABLE_WALLET
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("CBitcoin Miner: generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock, true, NULL))
        return error("CBitcoin Miner: ProcessNewBlock, block not accepted");
...
#endif
{
    LogPrintf("CBitcoin Miner started %s\n");
    ...
    try {
        while (true) {
            ...
            if (isNextBlockFork) {
                if (!bForkModeStarted) {
                    LogPrintf("CBitcoin Miner: switching into fork mode\n");
                ...
                if (!pblocktemplate.get()) {
                    if (bFileNotFound) {
                        ...
                        LogPrintf("Error in CBitcoin Miner: Cannot create Fork Block\n");
                        return;
                    }
                }
                ...
                LogPrintf("Running CBitcoin Miner with %u forking transactions in block (%u bytes) and N = %d, K = %d\n",
                          pblock->vtx.size(),
                ...
                if (bForkModeStarted) {
                    LogPrintf("CBitcoin Miner: Fork is done - switching back to regular miner\n");
                ...
                {
                    if (GetArg("-mineraddress", "").empty()) {
                        LogPrintf("Error in CBitcoin Miner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                    } else {
                        // Should never reach here, because -mineraddress validity is checked in init.cpp
                        LogPrintf("Error in CBitcoin Miner: Invalid -mineraddress\n");
                    }
                    return;
                }
                pblock = &pblocktemplate->block;

                LogPrintf("Running CBitcoin Miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
                          ::GetSerializeSize(*pblock, SER_NETWORK, PROTOCOL_VERSION));
    ...
    catch (const boost::thread_interrupted&)
    {
        miningTimer.stop();
        c.disconnect();
        LogPrintf("CBitcoin Miner terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        miningTimer.stop();
        c.disconnect();
        LogPrintf("CBitcoin Miner runtime error: %s\n", e.what());
        return;
    }
    miningTimer.stop();
    c.disconnect();
