    ...
    while (file.good()) {
        ...
        if (fImportZKeys) {
            try {
                ...
                // Not a valid spending key, so carry on and see if it's a CBTC style address.
            }
        }
...
UniValue dumpprivkey(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey \"cbtcaddress\"\n"
            "\nReveals the private key corresponding to 'cbtcaddress'.\n"
            "Then the importprivkey can be used with this output\n"
            "\nArguments:\n"
            "1. \"cbtcaddress\"   (string, required) The cbtc address for the private key\n"
            "\nResult:\n"
...
UniValue z_exportwallet(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"filename\"    (string, required) The filename, saved in folder set by cbtcd -exportdir option\n"
            "\nResult:\n"
...
UniValue dumpwallet_impl(const UniValue& params, bool fHelp, bool fDumpZKeys)
{
    ...
    if (exportdir.empty()) {
        throw JSONRPCError(RPC_WALLET_ERROR, "Cannot export wallet until the cbtcd -exportdir option has been set");
    }
    ...
    // produce output
    file << strprintf("# Wallet dump created by Classic Bitcoin %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
