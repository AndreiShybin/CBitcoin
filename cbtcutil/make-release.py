...
def main(args=sys.argv[1:]):
    """
    Perform the final Classic Bitcoin release process up to the git tag.
    """
...
def gen_release_notes(release):
    sh_log('python', './cbtc/release-notes.py', '--version', release.novtext)
    sh_log(
        'git',
        'add',
        './doc/authors.md',
...
def update_debian_changelog(release):
    ...
    os.environ['DEBFULLNAME'] = 'Classic Bitcoin'
    sh_log(
        'debchange',
...
def patch_README(release, releaseprev):
    with PathPatcher('README.md') as (inf, outf):
        ...
        assert firstline == 'Classic Bitcoin {}\n'.format(releaseprev.novtext), \
            repr(firstline)

        outf.write('Classic Bitcoin {}\n'.format(release.novtext))
        outf.write(inf.read())
...
def patch_gitian_linux_yml(release, releaseprev):
    ...
    with PathPatcher(path) as (inf, outf):
        ...
        assert secondline == 'name: "cbtc-{}"\n'.format(
            releaseprev.novtext
        ), repr(secondline)

        outf.write('name: "cbtc-{}"\n'.format(release.novtext))
        outf.write(inf.read())
...
def initialize_logging():
    ...
    logging.info('cbtc make-release.py debug log: %r', logname)
