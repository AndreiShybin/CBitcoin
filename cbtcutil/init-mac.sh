...
function main() {
    ...
    cat <<EOF
Classic Bitcoin - fetch-params.sh

This script will fetch the Classic Bitcoin (and Zcash) zkSNARK parameters and
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Classic Bitcoin zkSNARK parameters. Note that it is
...
if [ ! -f "$HOME/Library/Application Support/CBitcoin/cbitcoin.conf" ]; then
    echo "Creating cbitcoin.conf"
    mkdir -p "$HOME/Library/Application Support/CBitcoin/"
    echo "rpcuser=cbtcrpc" > ~/Library/Application\ Support/CBitcoin/cbitcoin.conf
    PASSWORD=$(cat /dev/urandom | env LC_CTYPE=C tr -dc 'a-zA-Z0-9' | fold -w 32 | head -n 1)
    echo "rpcpassword=$PASSWORD" >> "$HOME/Library/Application Support/CBitcoin/cbitcoin.conf"
    echo "Complete!"
fi
