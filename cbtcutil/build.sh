...
#   MAKE=gmake ./cbtcutil/build.sh -j$(nproc)
if [[ -z "${MAKE-}" ]]; then
    MAKE=make
fi

# Allow overrides to $BUILD and $HOST for porters. Most users will not need it.
#   BUILD=i686-pc-linux-gnu ./cbtcutil/build.sh
if [[ -z "${BUILD-}" ]]; then
...
$0 [ --enable-lcov || --disable-tests ] [ --disable-mining ] [ --disable-rust ] [ --enable-proton ] [ --disable-libs ] [ MAKEARGS... ]
  Build Classic Bitcoin and most of its transitive dependencies from
  source. MAKEARGS are applied to both dependencies and Classic Bitcoin itself.

  If --enable-lcov is passed, Classic Bitcoin is configured to add coverage
  instrumentation, thus enabling "make cov" to work.
  If --disable-tests is passed instead, the Classic Bitcoin tests are not built.

  If --disable-mining is passed, Classic Bitcoin is configured to not build any mining
  code. It must be passed after the test arguments, if present.

  If --disable-rust is passed, Classic Bitcoin is configured to not build any Rust language
  assets. It must be passed after test/mining arguments, if present.

  If --enable-proton is passed, Classic Bitcoin is configured to build the Apache Qpid Proton
  library required for AMQP support. This library is not built by default.
  It must be passed after the test/mining/Rust arguments, if present.

  If --disable-libs is passed, Classic Bitcoin is configured to not build any libraries like
  'libzcashconsensus'.
EOF
    exit 0
fi
